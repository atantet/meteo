...
def assembler_prevision_48h(
...
) -> tuple[Prevision48h, VigilanceDepartementDP]:
    """Fetch AROME + PE-AROME + DPVigilance et assemble la 48 h + la Vigilance.

    ``run_proba_utc`` : run PE-AROME (cycle propre) ; défaut = ``run_utc``. Lève les
    erreurs ``*IndisponibleError`` des sources (la cascade reste gérée par l'appelant
    comme pour le webservice).
    """
    ...
    vigilance = recuperer_vigilance_dp(departement, basic=basic, session=session)
    df = assembler_df_48h(arome, proba_6h, vigilance.tranches_orages())
