...
STATION_NAME_LABEL = {"DPObs": "Nom_usuel", "DPPaquetObs": "Nom_usuel", "DPClim": "nom"}

# Étiquette des identifiants des stations
ID_STATION_LABEL = {"DPObs": "Id_station", "DPPaquetObs": "Id_station", "DPClim": "id"}
...
OUVERT_STATION_LABEL = {"DPObs": None, "DPPaquetObs": None, "DPClim": "posteOuvert"}

# Étiquette du drapeau du caractère public des stations
PUBLIC_STATION_LABEL = {"DPObs": None, "DPPaquetObs": None, "DPClim": "postePublic"}

# Étiquette du type de station
TYPE_STATION_LABEL = {"DPObs": None, "DPPaquetObs": None, "DPClim": "typePoste"}

# Étiquette de l'indice temporel
TIME_LABEL = {"DPObs": "validity_time", "DPPaquetObs": "validity_time", "DPClim": "DATE"}
...
VARIABLES_CONVERSION_UNITES = {
    ...
    "DPClim": {
        "rayonnement_global": lambda x: x * 1.0e4,
...
class Client:
    ...
    def __init__(self, api: str, application_id: str | None = None) -> None:
        ...
        if api not in AVAILABLE_APIS:
            raise ValueError(f"Choix invalide: {api}. Les choix possibles sont: {AVAILABLE_APIS}")
        self.api = api
...
def response_text_to_frame(client, response, **kwargs):
    try:
        ...
        df = pd.read_json(StringIO(response.text)).set_index(client.id_station_label)
...
def demande(client, section, params=None, frequence=None, verify=False):
    ...
    response = client.request("GET", url, params=params, verify=verify)
...
def get_filepath_liste_stations_nn(
    client, ref_station_name, df_liste_stations, frequence=None, id_departement=None
):
...
def get_filepath_donnee_periode(
    client,
    ref_station_name,
    df_liste_stations=None,
    date_deb_periode=None,
    ...
    filename += (
        f"_{str_ref_station_name}_{str_nn}"
...
def compiler_donnee_des_stations_date(client, df_liste_stations, date, frequence=None):
...
def compiler_commandes_des_stations_periode(
    client, df_liste_stations, date_deb_periode, date_fin_periode, frequence=None
):
    params = {"date-deb-periode": date_deb_periode, "date-fin-periode": date_fin_periode}
    id_commandes = {}
    for id_station in df_liste_stations.index:
        # Paramètres définissant la station, la date et le format des données
        ...
        id_commandes[id_station] = response.json()["elaboreProduitAvecDemandeResponse"]["return"]
...
def compiler_telechargement_des_stations_periode(
    ...
    id_commandes = compiler_commandes_des_stations_periode(
        client, df_liste_stations, date_deb_periode, date_fin_periode, frequence=frequence
    )
...
def compiler_donnee_des_departements(client, df_liste_stations, frequence=None):
    id_departements = liste_id_stations_vers_liste_id_departements(df_liste_stations)
    ...
    for id_dep in id_departements:
        ...
        ).set_index([client.id_station_donnee_label, client.time_label])
    ...
    try:
        ...
        warnings.warn(f"les stations {', '.join(indices_manquants.astype(str))} manquent.")
...
def filtrer_stations_valides(client, df_brute):
    validite = (
        df_brute[client.ouvert_station_label]
...
def renommer_variables(client: Client, df: pd.DataFrame, frequence: str) -> pd.DataFrame:
...
def inserer_noms_stations(client, df, df_liste_stations):
    ...
    liste_noms_stations = [
        df_liste_stations.loc[_, client.station_name_label] for _ in id_stations_df
    ]
...
def get_str_date(date):
    try:
        ...
        s_date = (
            pd.Timestamp(date).isoformat().replace("+00:00", "Z").replace("-", "").replace(":", "")
        )
    return s_date
...
def localisation_temps(df, tz=TZ):
    ...
    index = [df.index.levels[0], pd.DatetimeIndex(df.index.levels[1], tz=TZ)]
...
def convertir_unites(client: Client, df: pd.DataFrame) -> pd.DataFrame:
    ...
    for variable, s in df.items():
        # variable est Hashable côté pandas-stubs, mais c'est bien un str ici.
        df.loc[:, variable] = client.variables_conversion_unites[str(variable)](s)
    return df
